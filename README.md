...
| **Asset Type** | AML Alert |
| **Control Record** | AML Alert Monitoring State |
...
| POST | `/v1/aml-alert-monitoring-state/initiate` | Initiate |
| GET | `/v1/aml-alert-monitoring-state` | Retrieve (list) |
| GET | `/v1/aml-alert-monitoring-state/{crId}/retrieve` | Retrieve |
| PUT | `/v1/aml-alert-monitoring-state/{crId}/update` | Update |
| PUT | `/v1/aml-alert-monitoring-state/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/aml-alert-monitoring-state/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-anti-money-laundering:0.1.0 .
```
...
Exposed through the platform Gateway at path prefix `/sd-anti-money-laundering` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
